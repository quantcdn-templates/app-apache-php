...
# Configure Postfix as a simple mail relay if SMTP relay mode is NOT enabled
# (Replaces ssmtp - postfix provides /usr/sbin/sendmail for PHP mail())
if [ -n "$QUANT_SMTP_HOST" ] && [ "$QUANT_SMTP_RELAY_ENABLED" != "true" ]; then
    echo "Configuring Postfix (simple relay) with host: $QUANT_SMTP_HOST"

    # Determine domain from QUANT_SMTP_FROM
    ...
    # Configure postfix for simple relay (equivalent to ssmtp behavior)
    ...
    if postfix status >/dev/null 2>&1; then
        echo "Postfix simple relay configured and started"
